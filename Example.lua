local library = (function()
    ...
    local function format_text(str) 
        return lib.theme.lowercase and string.lower(tostring(str)) or tostring(str) 
    end

    function lib:update_theme()
    ...
    run.RenderStepped:Connect(function()
        if lib.theme.rainbow then
            local hue = tick() % 5 / 5
            lib.theme.accent = Color3.fromHSV(hue, 0.7, 1)
            lib:update_theme()
        end
    end)
    ...
    function lib:init(name)
        ...
        function lib:create_category(cat_name, pos)
            local cat = {}
...
local ui = library:init("shadowui")
ui:create_watermark()

local combat = ui:create_category("combat", UDim2.new(0, 50, 0, 60))
combat:add_module("kill aura", function() end)

local visual = ui:create_category("visual", UDim2.new(0, 280, 0, 60))
visual:add_module("esp", function() end)

local client = ui:create_category("client", UDim2.new(0, 510, 0, 60))
local interface = client:add_module("interface", function() end)
interface:add_slider("blur size", 0, 30, library.theme.blur_size, function(v) library.theme.blur_size = v library:update_theme() end)
interface:add_slider("transparency", 0, 100, library.theme.transparency * 100, function(v) library.theme.transparency = v / 100 library:update_theme() end)
interface:add_toggle("rainbow accent", library.theme.rainbow, function(v) library.theme.rainbow = v end)
interface:add_toggle("lowercase", library.theme.lowercase, function(v) library.theme.lowercase = v end)
